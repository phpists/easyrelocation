<!DOCTYPE html>
<html lang="en">
    <head>
    <meta charset="utf-8" >
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0" >
    <link
        rel="stylesheet"
        href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css?_v=20231202142830"/>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css?_v=20231202142830" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
    <link rel="stylesheet" href="css/style.min.css?_v=20231202142830">
    <title>Наши работы</title>
</head>
    <body>
        <header class="header">
    <div class="container">
        <div class="header__container">
            <div class="row">
                <div class="col-2 col-sm-2 col-md-1 d-flex align-items-center d-lg-none">
                    <div class="header__body">
                        <div class="header__burger">
                            <span></span>
                        </div>
                        <nav class="header__burger-menu">
                            <div class="container-sm container-md container-lg">
                                <ul class="header__burger-list">
                                    <li class="header__menu-item">
                                        <a href="index.php" class="header__menu-link">О НАС</a>
                                    </li>
                                    <li class="header__menu-item">
                                        <a href="our-works.php" class="header__menu-link">НАШИ РАБОТЫ</a>
                                    </li>
                                    <li class="header__menu-item">
                                        <a href="contacts.php" class="header__menu-link">КОНТАКТЫ</a>
                                    </li>
                                </ul>
                            </div>
                        </nav>
                    </div> 
                    <!-- <div class="header__burger">
                        <nav class="header__nav header__burger-nav">
                            <ul class="header__menu header__burger-menu">
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">Гражданство</a>
                                </li>
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">ВНЖ</a>
                                </li>
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">Сравнение</a>
                                </li>
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">Недвижимость</a>
                                </li>
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">Истории клиентов</a>
                                </li>
                                <li class="header__menu-item">
                                    <a href="#" class="header__menu-link">О нас</a>
                                </li>
                            </ul>
                        </nav>
                    </div>-->
                </div> 
                <div class="col-9 col-sm-5 col-md-4 col-lg-2 d-flex">
                    <div class="logo">
    <a href="https://pergolaconstruct.ru/" class="logo__link">
        <img src="img/logo.svg" alt="logo">
    </a>
</div>
                </div>
                <div class="justify-content-end justify-content-lg-between d-none d-sm-flex col-sm-5 col-md-7 col-lg-10 d-flex align-items-center">
                    <nav class="header__nav">
                        <ul class="header__menu">
                            <li class="header__menu-item">
                                <a href="index.php" class="header__menu-link">О НАС</a>
                            </li>
                            <li class="header__menu-item">
                                <a href="our-works.php" class="header__menu-link">НАШИ РАБОТЫ</a>
                            </li>
                            <li class="header__menu-item">
                                <a href="contacts.php" class="header__menu-link">КОНТАКТЫ</a>
                            </li>
                        </ul>
                    </nav>
                    <div class="header__contact">
                        <a href="#" class="header__contact-viber">
                            <xml version="1.0" encoding="iso-8859-1">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 50 50" width="50px" height="50px">
                                <path d="M44.78,13.155c-0.78-2.786-2.325-4.99-4.591-6.55c-2.86-1.971-6.149-2.669-8.989-3.093c-3.931-0.586-7.491-0.669-10.882-0.254c-3.181,0.39-5.574,1.011-7.76,2.016c-4.281,1.967-6.851,5.15-7.637,9.461c-0.381,2.092-0.638,3.985-0.787,5.788c-0.343,4.172-0.032,7.863,0.952,11.285c0.959,3.336,2.635,5.719,5.123,7.284c0.635,0.399,1.446,0.688,2.231,0.966c0.446,0.158,0.879,0.311,1.235,0.476c0.328,0.152,0.328,0.181,0.325,0.455c-0.028,2.372,0,7.019,0,7.019L14.006,49h1.783l0.291-0.28c0.188-0.181,4.605-4.447,6.177-6.164l0.214-0.241c0.271-0.31,0.271-0.31,0.55-0.315c2.124-0.042,4.295-0.124,6.452-0.241c2.614-0.143,5.642-0.395,8.493-1.585c2.61-1.09,4.514-2.819,5.659-5.14c1.194-2.421,1.903-5.043,2.169-8.016C46.26,21.793,45.929,17.258,44.78,13.155z M35.382,33.479c-0.654,1.069-1.633,1.809-2.786,2.291c-0.842,0.352-1.702,0.278-2.532-0.074c-6.968-2.948-12.43-7.595-16.042-14.274c-0.744-1.375-1.262-2.874-1.855-4.328c-0.122-0.299-0.114-0.65-0.167-0.976c0.051-2.35,1.853-3.671,3.673-4.072c0.696-0.154,1.311,0.092,1.828,0.585c1.428,1.363,2.557,2.943,3.408,4.716c0.373,0.777,0.205,1.466-0.43,2.044c-0.131,0.12-0.269,0.232-0.411,0.339c-1.446,1.088-1.659,1.91-0.888,3.547c1.313,2.786,3.49,4.656,6.308,5.816c0.743,0.306,1.442,0.155,2.009-0.447c0.076-0.081,0.162-0.158,0.218-0.251c1.111-1.853,2.722-1.669,4.209-0.612c0.977,0.695,1.927,1.426,2.893,2.135C36.29,30.999,36.277,32.015,35.382,33.479z M26.143,15c-0.327,0-0.656,0.027-0.979,0.081c-0.546,0.092-1.06-0.277-1.151-0.822c-0.091-0.545,0.277-1.06,0.822-1.151C25.266,13.037,25.706,13,26.143,13C30.476,13,34,16.525,34,20.857c0,0.44-0.037,0.88-0.108,1.309C33.809,22.654,33.386,23,32.906,23c-0.055,0-0.111-0.004-0.167-0.014c-0.544-0.092-0.912-0.607-0.82-1.152C31.973,21.515,32,21.187,32,20.857C32,17.627,29.373,15,26.143,15z M31,21c0,0.552-0.448,1-1,1s-1-0.448-1-1c0-1.654-1.346-3-3-3c-0.552,0-1-0.448-1-1s0.448-1,1-1C28.757,16,31,18.243,31,21z M36.709,23.221C36.604,23.686,36.192,24,35.735,24c-0.073,0-0.148-0.008-0.222-0.025c-0.539-0.122-0.876-0.658-0.754-1.196c0.145-0.639,0.218-1.298,0.218-1.961c0-4.862-3.956-8.818-8.818-8.818c-0.663,0-1.322,0.073-1.961,0.218c-0.536,0.124-1.074-0.215-1.196-0.754s0.215-1.074,0.754-1.196C24.54,10.09,25.349,10,26.16,10c5.965,0,10.818,4.853,10.818,10.818C36.978,21.629,36.887,22.438,36.709,23.221z"/>
                            </svg>
                        </a>
                        <a href="#" class="header__contact-tg">
                            <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M25 2C37.703 2 48 12.297 48 25C48 37.703 37.703 48 25 48C12.297 48 2 37.703 2 25C2 12.297 12.297 2 25 2ZM32.934 34.375C33.357 33.077 35.339 20.141 35.584 17.592C35.658 16.82 35.414 16.307 34.936 16.078C34.358 15.8 33.502 15.939 32.509 16.297C31.147 16.788 13.735 24.181 12.729 24.609C11.775 25.014 10.873 25.456 10.873 26.096C10.873 26.546 11.14 26.799 11.876 27.062C12.642 27.335 14.571 27.92 15.71 28.234C16.807 28.537 18.056 28.274 18.756 27.839C19.498 27.378 28.061 21.648 28.676 21.146C29.29 20.644 29.78 21.287 29.278 21.79C28.776 22.292 22.898 27.997 22.123 28.787C21.182 29.746 21.85 30.74 22.481 31.138C23.202 31.592 28.387 35.07 29.168 35.628C29.949 36.186 30.741 36.439 31.466 36.439C32.191 36.439 32.573 35.484 32.934 34.375Z" fill="black"/>
                            </svg>
                        </a>
                        <a href="tel:+88006006193" class="header__contact-phone">8 800 600 61 93</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>
        <main>
            <section class="stg-section stg-section_margin stg-section_margin-small">
                <div class="container">
                    <div class="breadcrumbs">
    <nav class="breadcrumbs__nav">
        <a href="index.php" class="breadcrumbs__first-item">Главная</a>
        <span class="breadcrumbs__slash">/</span>
        <span class="breadcrumbs__basic-item">Контакты</span>
    </nav>
</div>
                </div>
            </section>
            <section class="stg-section stg-section_margin">
                <div class="container">
                    <div class="big-work-examples gallery">
    <div class="big-work-examples_desk d-none d-md-block">
        <div class="row mb-4">
            <div class="col-4 d-flex flex-column justify-content-between">
                <picture><source srcset="img/portfolio1.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio1.jpg" id="test-img" alt="Перголы Decolife Bioclimatic В600, B400; г. Москва, Фудмолл Депо, р-н Сыроварня [465]"></picture>
                <picture><source srcset="img/portfolio2.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio2.jpg" id="idimg"></picture>
            </div>
            <div class="col-5 d-flex flex-column justify-content-between">
                <picture><source srcset="img/portfolio3.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio3.jpg" id="img"></picture>
                <picture><source srcset="img/portfolio4.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio4.jpg" id="test"></picture>
            </div>
            <div class="col-3">
                <picture><source srcset="img/portfolio5.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio5.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties13.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties13.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties11.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties11.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties12.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties12.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties14.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties14.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties10.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties10.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/portfolio2.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio2.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col-4 d-flex flex-column justify-content-between">
                <picture><source srcset="img/portfolio1.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio1.jpg"></picture>
                <picture><source srcset="img/portfolio2.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio2.jpg"></picture>
            </div>
            <div class="col-5 d-flex flex-column justify-content-between">
                <!--<div class="big-work-examples__img-address-block">
                    <picture><source srcset="img/portfolio3.webp" type="image/webp"><img src="img/portfolio3.jpg" class="big-work-examples__img-without-address"></picture>
                    <div class="big-work-examples__img-address">
                        <div class="big-work-examples__img-address-text">
                            <p>Перголы Decolife Bioclimatic В600, B400</p>
                            <p>г. Москва, Фудмолл Депо, р-н Сыроварня [465]</p>
                        </div>
                    </div>
                </div>-->
                <picture><source srcset="img/portfolio3.webp" type="image/webp"><img src="img/portfolio3.jpg" class="big-work-examples__img"></picture>
                <picture><source srcset="img/portfolio4.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio4.jpg"></picture>
            </div>
            <div class="col-3">
                <picture><source srcset="img/portfolio5.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio5.jpg"></picture>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <picture><source srcset="img/varieties7.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties7.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties8.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties8.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties9.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties9.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/portfolio5.webp" type="image/webp"><img class="big-work-examples__img" src="img/portfolio5.jpg"></picture>
            </div>
        </div>
    </div>
    <div class="big-work-examples_mob d-block d-md-none">
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties13.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties13.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties11.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties11.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties14.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties14.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties10.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties10.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties7.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties7.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties8.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties8.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties13.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties13.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties11.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties11.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties14.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties14.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties10.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties10.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties7.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties7.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties8.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties8.jpg"></picture>
            </div>
        </div>
        <div class="row mb-4">
            <div class="col">
                <picture><source srcset="img/varieties13.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties13.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties11.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties11.jpg"></picture>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <picture><source srcset="img/varieties14.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties14.jpg"></picture>
            </div>
            <div class="col">
                <picture><source srcset="img/varieties10.webp" type="image/webp"><img class="big-work-examples__img" src="img/varieties10.jpg"></picture>
            </div>
        </div>
    </div>
</div>
                </div>
            </section>
            <section class="stg-section stg-section_margin stg-bg-accent">
                <div class="container">
                    <div class="connection">
    <div class="connection__img">
        <picture><source srcset="img/man.webp" type="image/webp"><img src="img/man.png" alt="man" class="connection__img-elem"></picture>
    </div>
    <div class="connection__header">
        <div class="connection__pretitle">
            за 5 минут
        </div>
        <h2 class="connection__title stg-h2">
            Узнайте стоимость перголы
        </h2>
    </div>
    <div class="connection__content">
        <div class="connection__descr">
            Оставьте свой номер телефона, мы перезвоним вам в течении пяти минут и рассчитаем стоимость перголы
        </div>
        <div class="connection__wrap">
            <div>
                <p class="connection__guarantee-text">* С Гарантией от производителя</p>
                <ul class="connection__list">
                    <li class="connection__list-item">по вашим размерам</li>
                    <li class="connection__list-item">с монтажом</li>
                </ul>
            </div>
            <div class="connection__contact">
                <a href="#" class="connection__contact-viber">
                    <xml version="1.0" encoding="iso-8859-1"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 50 50" width="50px" height="50px"><path d="M44.78,13.155c-0.78-2.786-2.325-4.99-4.591-6.55c-2.86-1.971-6.149-2.669-8.989-3.093c-3.931-0.586-7.491-0.669-10.882-0.254c-3.181,0.39-5.574,1.011-7.76,2.016c-4.281,1.967-6.851,5.15-7.637,9.461c-0.381,2.092-0.638,3.985-0.787,5.788c-0.343,4.172-0.032,7.863,0.952,11.285c0.959,3.336,2.635,5.719,5.123,7.284c0.635,0.399,1.446,0.688,2.231,0.966c0.446,0.158,0.879,0.311,1.235,0.476c0.328,0.152,0.328,0.181,0.325,0.455c-0.028,2.372,0,7.019,0,7.019L14.006,49h1.783l0.291-0.28c0.188-0.181,4.605-4.447,6.177-6.164l0.214-0.241c0.271-0.31,0.271-0.31,0.55-0.315c2.124-0.042,4.295-0.124,6.452-0.241c2.614-0.143,5.642-0.395,8.493-1.585c2.61-1.09,4.514-2.819,5.659-5.14c1.194-2.421,1.903-5.043,2.169-8.016C46.26,21.793,45.929,17.258,44.78,13.155z M35.382,33.479c-0.654,1.069-1.633,1.809-2.786,2.291c-0.842,0.352-1.702,0.278-2.532-0.074c-6.968-2.948-12.43-7.595-16.042-14.274c-0.744-1.375-1.262-2.874-1.855-4.328c-0.122-0.299-0.114-0.65-0.167-0.976c0.051-2.35,1.853-3.671,3.673-4.072c0.696-0.154,1.311,0.092,1.828,0.585c1.428,1.363,2.557,2.943,3.408,4.716c0.373,0.777,0.205,1.466-0.43,2.044c-0.131,0.12-0.269,0.232-0.411,0.339c-1.446,1.088-1.659,1.91-0.888,3.547c1.313,2.786,3.49,4.656,6.308,5.816c0.743,0.306,1.442,0.155,2.009-0.447c0.076-0.081,0.162-0.158,0.218-0.251c1.111-1.853,2.722-1.669,4.209-0.612c0.977,0.695,1.927,1.426,2.893,2.135C36.29,30.999,36.277,32.015,35.382,33.479z M26.143,15c-0.327,0-0.656,0.027-0.979,0.081c-0.546,0.092-1.06-0.277-1.151-0.822c-0.091-0.545,0.277-1.06,0.822-1.151C25.266,13.037,25.706,13,26.143,13C30.476,13,34,16.525,34,20.857c0,0.44-0.037,0.88-0.108,1.309C33.809,22.654,33.386,23,32.906,23c-0.055,0-0.111-0.004-0.167-0.014c-0.544-0.092-0.912-0.607-0.82-1.152C31.973,21.515,32,21.187,32,20.857C32,17.627,29.373,15,26.143,15z M31,21c0,0.552-0.448,1-1,1s-1-0.448-1-1c0-1.654-1.346-3-3-3c-0.552,0-1-0.448-1-1s0.448-1,1-1C28.757,16,31,18.243,31,21z M36.709,23.221C36.604,23.686,36.192,24,35.735,24c-0.073,0-0.148-0.008-0.222-0.025c-0.539-0.122-0.876-0.658-0.754-1.196c0.145-0.639,0.218-1.298,0.218-1.961c0-4.862-3.956-8.818-8.818-8.818c-0.663,0-1.322,0.073-1.961,0.218c-0.536,0.124-1.074-0.215-1.196-0.754s0.215-1.074,0.754-1.196C24.54,10.09,25.349,10,26.16,10c5.965,0,10.818,4.853,10.818,10.818C36.978,21.629,36.887,22.438,36.709,23.221z"/></svg>
                </a>
                <a href="#" class="connection__contact-tg">
                    <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M25 2C37.703 2 48 12.297 48 25C48 37.703 37.703 48 25 48C12.297 48 2 37.703 2 25C2 12.297 12.297 2 25 2ZM32.934 34.375C33.357 33.077 35.339 20.141 35.584 17.592C35.658 16.82 35.414 16.307 34.936 16.078C34.358 15.8 33.502 15.939 32.509 16.297C31.147 16.788 13.735 24.181 12.729 24.609C11.775 25.014 10.873 25.456 10.873 26.096C10.873 26.546 11.14 26.799 11.876 27.062C12.642 27.335 14.571 27.92 15.71 28.234C16.807 28.537 18.056 28.274 18.756 27.839C19.498 27.378 28.061 21.648 28.676 21.146C29.29 20.644 29.78 21.287 29.278 21.79C28.776 22.292 22.898 27.997 22.123 28.787C21.182 29.746 21.85 30.74 22.481 31.138C23.202 31.592 28.387 35.07 29.168 35.628C29.949 36.186 30.741 36.439 31.466 36.439C32.191 36.439 32.573 35.484 32.934 34.375Z" fill="black"/>
                    </svg>
                </a>
                <a href="tel:+88006006193" class="connection__contact-phone">8 800 600 61 93</a>
            </div>
        </div>
        <div class="connection__ctrl">
            <input type="phone" class="connection__input" placeholder="номер телефона">
            <div class="btn-panel btn-panel_white btn-panel_without-margin">
                <button class="stg-btn">Узнать</button>
                <div class="decoration"></div>
            </div>
        </div>
    </div>
</div>
                </div>
            </section>
        </main>
        <footer class="footer">
     <div class="container">
         <div class="footer__container d-flex">
            <div class="footer__logo order-3 order-lg-0">
                <a href="#" class="footer__logo-link">
                    <img src="img/logo-big.svg" alt="logo-big">
                </a>
            </div>
            <div class="footer__column footer__column_first order-2 order-lg-1">
                <div class="footer__column-item mb-4">
                    <div class="footer__list-title">ПЕРГОЛЫ</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Биоклиматические перголы
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Тентовые перголы
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Зимний сад
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="footer__column-item">
                    <div class="footer__list-title">МАРКИЗЫ</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Кассетные маркизы Premium
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Кассетные маркизы Standart
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Полукассетные маркизы
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Открытые маркизы
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Витринные маркизы
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="footer__column footer__column_second order-1 order-lg-2">
                <div class="footer__column-item mb-4">
                    <div class="footer__list-title">ВЕРТИКАЛЬНЫЕ СИСТЕМЫ</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Zip системы
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Screen системы
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Антимоскитные системы
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="footer__column-item">
                    <div class="footer__list-title">ТКАНИ</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Serge Ferrari (Soltis)
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Copaco (Screen)
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Sattler (Тентовые ткани)
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Sattler (Акриловые ткани)
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Gaviota (Акриловые ткани)
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="footer__column footer__column_third order-0 order-lg-3">
                <div class="footer__column-item mb-4">
                    <div class="footer__list-title">ОСТЕКЛЕНИЕ ТЕРРАС</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Подъемное гильотинное остекление
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Подъемное гильотинное остекление с терморазрывом
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Раздвижное безрамное остекление
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Раздвижное остекление со стеклопакетом
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Ограждение стеклянное (триплекс)
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="footer__column-item">
                    <div class="footer__list-title">ПОРТАЛЫ, ОКНА, ДВЕРИ</div>
                    <ul class="footer__list">
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Панорамные раздвижные двери HS
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Панорамные складные двери FS (гармошка)
                            </a>
                        </li>                        
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Алюминиевые окна с терморазрывом
                            </a>
                        </li>
                        <li class="footer__list-item">
                            <a href="" class="footer__list-item-link footerLink" data-modal>
                                Алюминиевые окна без терморазрыва
                            </a>
                        </li>
                    </ul>
                </div>
            </div> 

        </div>
     </div>
</footer>
        <div class="modal">
    <div class="modal__dialog">
        <div class="modal__content">
            <form action="form.php" method="post">
                <div class="modal__close" data-close>&times;</div>
                <input required placeholder="Ваше имя" name="name" type="text" class="modal__input">
                <input required placeholder="Ваша фамилия" name="surname" type="text" class="modal__input">
                <input required placeholder="Ваш номер телефона" name="phone" type="phone" class="modal__input">
                <input required placeholder="Ваше E-mail" name="email" type="email" class="modal__input">
                <input required placeholder="Ваедите промокод" name="promo" type="text" class="modal__input">
                <button type="submit" class="stg-btn stg-btn_big modal__btn">Оставить Заявку</button>
            </form>
        </div>
    </div>
</div>
        <div class="img-model" id="imgModal">
    <span class="img-model__close" id="imgModalClose">&times;</span>
    <img class="img-model__img" id="imageInModal">
    <div class="img-model__caption" id="captionInModal"></div>
</div>
        <script src="https://code.jquery.com/jquery-3.7.1.slim.min.js?_v=20231202142830" integrity="sha256-kmHvs0B+OpCW5GVHUNjv9rOmY0IvSIRcf7zGUDTDQM8=" crossorigin="anonymous"></script>
        <script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js?_v=20231202142830"></script>
        <script src="js/app.min.js?_v=20231202142830"></script>
    </body>
</html>
